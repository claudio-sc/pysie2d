...
| [multipole-spec.md](multipole-spec.md) | v0.7 code-spec for the multipole decomposition of the scattered field: the measured Mie relation, the three guards the legacy code lacked, and the full implementation. | **Historical. The authority on intent.** Draft 2, implemented in v0.7. |
| [v1.0-external-validation.md](v1.0-external-validation.md) | v1.0 high-level spec: the comparison contract, MEEP and dolfinx as the two anchors split by regime, the five gates, and the three-layer packaging. Renumbered from v0.7 on 20 Sep 2026, from v0.8 on 20 Sep 2026 when multiparticle took v0.8, and from v0.9 on 20 Sep 2026 when the owner decided this milestone ships as the 1.0 release itself rather than a step before it. | **Live.** Scoping; no code-spec yet. |
| [multiparticle-handoff.md](multiparticle-handoff.md) | The high-level guide for the arbitrary-multiparticle extension: what the legacy code gives, what it assumes, and what a code-spec must decide. | **Live.** A guide for a code-spec, not a code-spec. |
| [multiparticle-spec.md](multiparticle-spec.md) | v0.8 code-spec for the finite cluster of arbitrary particles: the coupled system and its cross-blocks, the convention bridge, absolute cross-sections, and the two-cylinder addition-theorem anchor. Its §2 records where it supersedes the handoff. | **Live**, draft 1. |
