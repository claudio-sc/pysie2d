...
| [v0.6-architecture.md](v0.6-architecture.md) | v0.6 doc A — decisions, scope, the `Parametrisation` object, the API break, the non-goals. | **Live.** Decisions taken, nothing implemented. |
| [parametrisation-spec.md](parametrisation-spec.md) | v0.6 code-spec for the smooth `Parametrisation`: Newton inversion replacing `np.interp`, closed-form boundary derivatives, the near-uniform density, the mild-star homotopy anchor. | **Live**, draft 1. Nothing implemented; its §8 measurements run first. |
