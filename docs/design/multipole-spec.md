...
carries a v0.7 multipole entry and a v1.0 external-validation entry, and that
`docs/design/v1.0-external-validation.md` exists. If it does not, you are on the
...
- The anchor is **Mie, on a circle**. For a non-circular shape the only check
  ...
  external anchor for non-circular shapes is the v1.0 milestone.
