...
def test_self_green_in_cladding(pol):
    ...
    # at n_pts = 240, which closes to round-off. At d = 2a the default reference
    # order converges like 4^(-n) and needs no override *(measured here: Re
    # 8.1e-14, Im 2.4e-14)*. Neither the anchor nor the rate depends on n_clad.
    geom = Geometry.gielis(rad=RAD, n_pts=240, m=0)
    mat = Material(n_core=SCALED["n_core"], n_clad=SCALED["n_clad"], pol=pol)
...
def _ellipse_map(b):
    """Near-uniform arc-length map of the §10 ellipse at semi-axis factor ``b``."""
    return Parametrisation.gielis(
        rad=RAD, a=1.0, b=b, m=4, n1=2.0, n2=2.0, n3=2.0, n_core=QNM_N_CORE
    )


def test_shape_derivative_of_m_depends_on_the_frozen_map():
    """§10: under a smooth map, ∂M/∂p is second order either way, and differs.

    v0.5 froze the node set because re-inverting arc length through
    ``np.interp`` put an O(h) term into the difference (rate 2.7 against 100).
    A smooth map removes that term: rebuilding the map at ``b ± h`` is second
    order too *(measured: 100.0 and 100.0)*. What it does instead is
    differentiate the node gauge along with the shape — the two derivatives
    differ by 20 % of ‖∂M/∂b‖ *(measured 0.200)* — so an adjoint quotient that
    mixed them would take ∂M/∂p and ∂M/∂λ on different discretisations. That
    is why the map, not the rate, is what §10 still freezes.

    Rate bound 50: the plateau carries the O(h⁴) term too (relatively 1e-2), so
    a factor of two around the ideal is the honest band. Difference bound 0.05:
    a quarter of the measured 0.200, and nine decades above the round-off at
    which a map that silently stopped being rebuilt would put it.
    """
    lam = 700.0 + 8.0j  # complex λ: the QNM path, not a real fast path
    n_pts = 60
    frozen_map = _ellipse_map(1.20)

    def deriv(h, frozen):
        mats = []
        for sign in (+1.0, -1.0):
            b = 1.20 + sign * h
            par = frozen_map if frozen else _ellipse_map(b)
            geo = Geometry.gielis(RAD, n_pts, m=4, b=b, parametrisation=par)
            mats.append(
    ...
    for frozen in (True, False):
        ref = deriv(1.0e-6, frozen)
        coarse = np.linalg.norm(deriv(1.0e-3, frozen) - ref)
        fine = np.linalg.norm(deriv(1.0e-4, frozen) - ref)
        assert coarse / fine > 50.0, (frozen, coarse / fine)
        refs[frozen] = ref

    gauge = np.linalg.norm(refs[True] - refs[False]) / np.linalg.norm(refs[True])
    assert gauge > 0.05
...
def test_wavelength_over_ds_uses_the_real_part_and_the_worst_node(circle):
    ...
    # A frozen arc-length map on a perturbed shape is the case where Δs stops
    # being uniform. R must then track max(Δs), so it can only fall relative to
    # the map equidistributed for that shape *(measured 24.8 against 28.2)*.
    base = _ellipse_map(1.20)
    stretched = Geometry.gielis(RAD, 200, m=4, b=1.60, parametrisation=base)
    equidistributed = Geometry.gielis(
        RAD, 200, m=4, b=1.60, parametrisation=_ellipse_map(1.60)
    )
...
def test_wavelength_over_ds_flags_an_elongated_shape_as_under_resolved():
    ...
    Measured on uniform arc length, where every Δs is equal and R is a property
    of the perimeter alone; on the default uniform-θ map the same ellipse reads
    9.2, because θ-spacing stretches the flanks.
    """
    mat = Material(n_core=QNM_N_CORE, pol=2)
    aspect3 = Parametrisation.gielis(
        rad=RAD, a=1.0, b=3.0, m=4, n1=2.0, n2=2.0, n3=2.0, n_core=QNM_N_CORE
    )
    circle_r = wavelength_over_ds(Geometry.gielis(RAD, 200, m=0), mat, 700.0)
    ellipse_r = wavelength_over_ds(
        Geometry.gielis(RAD, 200, m=4, b=3.0, parametrisation=aspect3), mat, 700.0
    )
    ...
    recovered = wavelength_over_ds(
        Geometry.gielis(RAD, 426, m=4, b=3.0, parametrisation=aspect3), mat, 700.0
    )
...
def test_sensitivity_step_is_in_the_parameters_own_units():
    ...
    the cancellation floor of the difference quotient — ε·rad/h ≈ 4e-9, since
    ∂M/∂rad ~ M/rad and h is in nm *(measured 3.8e-10)* — not a fitted number.
    Bound 1e-8.
    """
    geom = Geometry.gielis(rad=RAD, n_pts=40, m=0)
    mat = Material(n_core=QNM_N_CORE, n_clad=1.0, pol=2)
    res = QNMSolver(geom, mat).modes(z_lo=520 + 15j, z_hi=545 + 40j)
    par = res.geometry.parametrisation

    def per_nm(delta):
        return Geometry.gielis(rad=RAD + delta, n_pts=40, m=0, parametrisation=par), mat

    def per_ten_nm(delta):
        return (
            Geometry.gielis(rad=RAD + 10.0 * delta, n_pts=40, m=0, parametrisation=par),
            mat,
        )

    d_nm = res.sensitivity(per_nm, step=SHAPE_STEP)[0]
    d_ten = res.sensitivity(per_ten_nm, step=SHAPE_STEP)[0]
    assert d_ten == pytest.approx(10.0 * d_nm, rel=1.0e-8)
...
def test_sensitivity_degenerate_dispatch_agrees_with_multiplicity():
    ...
    geom = Geometry.gielis(rad=RAD, n_pts=40, m=0)
    mat = Material(n_core=QNM_N_CORE, n_clad=1.0, pol=2)
    res = QNMSolver(geom, mat).modes(z_lo=745 + 2j, z_hi=775 + 15j)
    par = res.geometry.parametrisation
    ...
    got = res.sensitivity(
        lambda d: (
            Geometry.gielis(rad=RAD + d, n_pts=40, m=0, parametrisation=par),
            mat,
        )
    )
...
def test_sensitivity_left_vector_is_not_conj_of_the_right_one():
    ...
    # *(measured 1.16 and 4.5e-16 at n_pts = 40)*
...
def test_geometry_without_a_node_set_is_allowed_but_cannot_be_differentiated():
    """§10: `parametrisation` is optional to store, and mandatory to differentiate.
    ...
    scattering — the whole solver runs without ever reading the map. But a
    shape derivative *is* taken on a frozen map (§10), so the frozen-map path
    must refuse such a geometry rather than fall back to anything.
    ...
    bare = Geometry(src.f, src.g, src.df, src.dg, src.ddf, src.ddg, rad=RAD)
    assert bare.parametrisation is None
    assert bare.nodes is None and bare.theta is None

    # The solver itself never reads the map, so a bare geometry scatters exactly
    # like the geometry it was copied from — bit for bit, since the arrays are
    # the same objects and no map enters the assembly.
    mat = Material(n_core=QNM_N_CORE, n_clad=1.0, pol=2)
